...
- ACTIVE — MapleStory-style pivot (replace stage-gated chapter map with persistent connected zones, mob respawn, HP/MP persistence across zones; stars repurposed as achievements). AG.1 shipped: zoneTransition() fade-load replaces STAGE CLEAR overlay between non-Warden zones, HP/MP/potions/RunStats persist via opts.zoneTransition in launchStage, walking back through a chapter unlocked via baseline visited-star recording. AG.2 next: bidirectional portals at zone edges + world-map fast travel. AG.3 after: mob respawn timer.
